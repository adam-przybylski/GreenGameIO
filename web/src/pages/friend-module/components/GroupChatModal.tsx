...
const GroupChatModal = ({ group, close, id }) => {
    ...
    const fetchChat = () => {
        api.getChat(group.chatId, setChat);
    }
    ...
    useEffect(() => {
        ...
        client.onConnect = (frame) => {
            console.log(frame);
            client.subscribe(`/topic/${group.chatId}`, (data) => {
    ...
    const sendMessage = () => {
        client.publish({
            destination: `/app/chat/${group.chatId}`,
            body: JSON.stringify({
                senderID: id,
                timestamp: new Date(),
                content: message
    ...
    return (
        <div id="modal-background" onClick={e => e.stopPropagation()}>
            <div id="modal">
                ...
                <div className="chat">
                { chat && chat.messages.map((x, i) => <p className={id == x.senderID ? "message right-user" : "message"}>{x.senderName}, {x.timestamp}: {x.content}</p>) }
                    { temp && temp.slice(1).map((x, i) => <p className={id == x.senderID ? "message right-user" : "message"}>{x.senderName}, {x.timestamp}: {x.content}</p>) }

export default {
  ...
  getAllUsers: (id, setUsers) => {
    fetch(
      `http://localhost:8081/friend-module/${id}/username-contains?username`
    )
      .then((response) => response.json())
  ...
  removeFriend: (id, friend) => {
    fetch(
      `http://localhost:8081/friend-module/${id}/remove-friend?receiverId=${friend.id}`,
      {
        method: "PATCH",
      }
    )
      .then((response) => {
        if (response.status == 200) {
          alert(
            `User \"${friend.name}\" has been removed from the friend list!`
          );
        }
      })
      .catch((err) => {
  ...
  addFriend: (id, friend) => {
    fetch(
      `http://localhost:8081/friend-module/${id}/send-friend-request?receiverId=${friend.id}`,
      {
        method: "PATCH",
      }
    )
  ...
  acceptFriendRequest: (id, request) => {
    fetch(
      `http://localhost:8081/friend-module/${id}/accept-friend-request?senderId=${request.id}`,
      {
        method: "PATCH",
      }
    )
  ...
  addGroupMember: (id, group, member) => {
    fetch(
      `http://localhost:8081/friend-module/${id}/groups/${group.id}/add-member?memberId=${member.id}`,
      {
        method: "POST",
      }
    )
  ...
  removeGroupMember: (id, groupID, memberID) => {
    fetch(
      `http://localhost:8081/friend-module/${id}/groups/${groupID}/remove-member?memberId=${memberID}`,
      {
        method: "DELETE",
      }
    )
